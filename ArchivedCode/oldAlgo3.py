...
#                x.append(lines[i][j][k])
            fftLines.append(FFTpos[lines[i][0][j],lines[i][1][j],lines[i][2][j]])
...
#    return fftLines
    print "Done!"
#    return fftLines, lines

    fftLines = np.reshape(fftLines, (np.shape(lines)[0], np.shape(lines)[1]))
#    fftLines = np.reshape(fftLines, np.shape(lines))
...
#cA, cD = algorithm3(pos, speed = 1)
#FFTpos, lines = algorithm3(pos)
FFTlines, lines = algorithm3(pos)
