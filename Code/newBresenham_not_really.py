...
def xArray(start, end, length, nbSteps):
    ...
    for k in range(len(stepLenArray)):
        for l in range(nbSteps):
            ...
            stepsArray[k, l] = np.int(start[k][0] + l*stepLenArray[k])
    return stepsArray
...
def yArray(xIncrements, start, ySlope, nbSteps):
    ...
    for i in range(len(start)):
        for j in range(nbSteps):
            if not ySlope[i] == -1:
                yArray[i, j] = np.int(xIncrements[i, j]*ySlope[i] + start[i][1])
            else:
                yArray[i, j] = np.int(xIncrements[i, j]*i + start[i][1])
...
#    zArray = np.zeros((nbCells, nbSteps), dtype = np.float)
    for i in range(len(start)):
        for j in range(nbSteps):
            if not zSlope[i] == -1:
                zArray[i, j] = np.int(xIncrements[i, j]*zSlope[i] + start[i][2])
            else:
                zArray[i, j] = np.int(xIncrements[i, j]*i + start[i][2])
...
def lineCreation(start, end):
    ...
    lenght = lineLen(start,end)
    print "X-Array..."    
    arrayx = xArray(start,end,lenght, nbSteps)
    print "Slopes..."    
    ...
    return lines#, arrayx, arrayy, arrayz, slopey, slopez


lines = lineCreation(start,end)
#lines, arrayx, arrayy, arrayz, slopey, slopez = lineCreation(start,end)
